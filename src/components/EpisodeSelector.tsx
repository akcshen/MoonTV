...
const EpisodeSelector: React.FC<EpisodeSelectorProps> = ({
  ...
  // 换源列表排序：当前源置顶。使用副本 + useMemo，避免在渲染中突变 props 且重复排序
                ...
                {sortedSources.map((source, index) => {
                  ...
                  return (
                    <button
                      ...
                      aria-label={`换源到 ${source.source_name} - ${source.title}`}
                      className={`w-full text-left flex items-start gap-3 px-2 py-3 rounded-lg transition-all select-none duration-200 relative
                      ${
                        isCurrentSource
